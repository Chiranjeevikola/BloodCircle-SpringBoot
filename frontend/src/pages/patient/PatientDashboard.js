...
export default function PatientDashboard() {
  ...
  const handleSwitchToDonor = async () => {
    ...
    try {
      ...
      if (updatedUser.has_donor_profile) {
        toast.success('Switched to Donor Dashboard.');
        navigate('/donor/dashboard');
      } else {
        toast.success('Role set to Donor. Please complete your profile.');
    ...
    <div className="page">
      <div className="container">
        ...
        <div className="card mt-2">
          ...
          <button onClick={handleSwitchToDonor} disabled={switching} className="btn btn-outline btn-sm">
            {switching ? 'Switching...' : 'Switch to Donor'}
          </button>
        </div>
      </div>
    </div>
