...
export default function DonorEditProfile() {
  ...
  useEffect(() => {
    API.get('/donor/profile').then(r => {
      ...
      setForm({
        full_name: d.full_name, phone: d.phone, blood_group: d.blood_group,
        date_of_birth: d.date_of_birth || '', gender: d.gender,
        city: d.city, state: d.state, pincode: d.pincode,
        last_donation_date: d.last_donation_date || '', medical_history: d.medical_history || '',
        is_available: d.is_available,
      });
    }).catch(() => navigate('/donor/register'));
  }, [navigate]);
  ...
  return (
    <div className="page">
      <div className="container" style={{ maxWidth: '700px' }}>
        <h1 className="section-title text-center">Edit Donor Profile</h1>
        <div className="card">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group"><label>Date of Birth</label><input type="date" className="form-control" value={form.date_of_birth} onChange={set('date_of_birth')} required /></div>
            <div className="form-row">
            ...
            <div className="form-group"><label>Last Donation Date</label><input type="date" className="form-control" value={form.last_donation_date} onChange={set('last_donation_date')} /></div>
            <div className="form-group"><label>Medical History</label><textarea className="form-control" value={form.medical_history} onChange={set('medical_history')} /></div>
            <div className="form-group"><label><input type="checkbox" checked={form.is_available} onChange={set('is_available')} /> Available to Donate</label></div>
